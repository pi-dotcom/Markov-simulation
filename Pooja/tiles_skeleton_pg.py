...
def get_customer(terrain_map,tiles,x,y):
    #num = cust_row.loc["customer_no"]
    global cust_counter,current_customers
    cust_counter += 1
    num = cust_counter 
    tile_size = 32
    ...
    img = tiles[row_start * tile_size : (row_start+1) * tile_size, column_start * tile_size : (column_start+1) * tile_size] 
    
    c = customer_pg.Customer(terrain_map,img,x,y)
    if not num in current_customers:
        current_customers[num] = c
    return c
...
if __name__ == "__main__":
    ...
    cust = get_customer(market ,tiles,15,10)
    cust2 = get_customer(market ,tiles,14,10)
    time_cnt = 0
    while True:
